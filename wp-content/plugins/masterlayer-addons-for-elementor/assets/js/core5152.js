...
(function($, window, document, undefined) {
    ...
    masterCarouselBox.prototype = {
        ...
        init: function() {
            ...
            this.responsive(); this.build(); this.eIO();
            return this;
        },
        eIO: function e() {
            var t = this;
        ...
        event: function() {
            ...
            // Drag & Drop
            if ( t.$elm.is('.has-custom-cursor') && $('.medixo-cursor').length ) {
                var cursor = $('.medixo-cursor');
            ...
            // Prevent click on dragging
            t.$elm.on('dragStart.flickity', function(event, pointer) {
                t.$elm.find('a').css('pointer-events', 'none');
            });
            t.$elm.on('dragEnd.flickity', function(event, pointer) {
                t.$elm.find('a').css('pointer-events', 'auto');
            });
...
(function($, window, document, undefined) {
    ...
    masterSlider.prototype = {
            ...
            t.params = {
                images: {
                    ...
                    zoomOut: {
                        ...
                        set: {
                            next: { y: 50, scale: 1, opacity: 0, zIndex: 2 },
                            prev: { y: 50, scale: 1, opacity: 0, zIndex: 2 },
                        },
                        in: { y: 0, scale: 1.1, opacity: 1, duration: 1 }
                    },
                    zoomIn: {
                        out: {
                            next: { y: -50, scale: 1, opacity: 0, duration: 1.5 },
                            prev: { y: -50, scale: 1, opacity: 0, duration: 1.5 },
                        },
                        set: {
                            next: { y: 50, scale: 1.2, opacity: 0, zIndex: 2 },
                            prev: { y: 50, scale: 1.2, opacity: 0, zIndex: 2 },
                        },
                        in: { y: 0, scale: 1.1, opacity: 1, duration: 1 }
                    },
                },
        ...
        event: function() {
            ...
            // Distortion
            if (t.args.distortion) {
                isCanvas = true;
                //t.$elm.addClass('is-webgl');
                var canvas = $('<canvas></canvas>');
                var wrapper = t.$elm.find('.bg-wrap').eq(0);
                
                //wrapper.find('.bg').remove(0);
                var imgs = [];
                t.$elm.find('.bg-wrap .bg').each(function(idx, el) {
                    var bg = $(el).css('background-image')
                    imgs[idx] = bg.replace('url(','').replace(')','').replace(/\"/gi, "");
                });
                switch (t.args.distortionType) {
                    case 'liquid1':
                    case 'liquid2':
                    case 'liquid3':
                    case 'liquid4':
                    case 'liquid5':
                        distortionEffect = new CanvasSlideshow({
                            displacementImage: t.args.disp,
                            images: imgs,
                            type: t.args.distortionType,
                            canvas: canvas.get(0),
                            wrapper: wrapper,
                            wacky: true
                        });
                    ...
                    case 'demo3':
                        ...
                        distortionEffect = new EffectDistortion2({
                            displacementImage: t.args.disp,
                            images: imgs,
                            type: t.args.distortionType,
                            canvas: canvas.get(0),
                    ...
                    default:
                        wrapper.append(canvas);
                        distortionEffect = new EffectDistortion({
                            displacementImage: t.args.disp,
                            images: imgs,
                            type: t.args.distortionType,
                            canvas: canvas.get(0),
            ...
            function updateStart() {
                if ($('.slider-event').length) { 
                    $('.slider-event').removeClass('transitionEnd active' + current + ' prev next');
                    for (let i = 0; i < slides.length; i++) {
                        $('.slider-event').removeClass('prev' + i);
                    }
                    $('.slider-event').addClass('transitionStart active' + newIndex + ' prev' + current + ' ' + direction)
                }
                
                if (t.$elm.find('.bg-wrap.masking').length) { 
                    t.$elm.find('.bg-wrap.masking .bg').eq(current).removeClass('active');
                    t.$elm.find('.bg-wrap.masking .bg').eq(newIndex).addClass('active');
                }
            ...
            function updateEnd() {
                ...
                if ($('.slider-event').length) { 
                    $('.slider-event').removeClass('transitionStart');
                    $('.slider-event').addClass('transitionEnd')
                }
            }
            ...
            function titleOut() {
                ...
                if (t.args.titleEffOut.eff == 'textEfx1') {
                    var target = slides.eq(current).find('.title .char');
                    return gsap.to( target, 1.5, { opacity: 0, ease: 'power1.inOut', stagger: 0.15});
                } else {
            ...
            function titleIn() {
                ...
                if (t.args.titleEffOut.eff == 'textEfx1') {
                    var target = slides.eq(newIndex).find('.title .char');
                    var delay = slides.eq(current).find('.title .char').length * 0.15 + 0.15;
                    gsap.set(target, {opacity: 0});
                    return gsap.to( target, 1.5, { opacity: 1, ease: 'expo.inOut', stagger: 0.15, delay: delay});
                } else {
            ...
            function canvasIn() {
                if (!isCanvas) return;
               	if (firstChange)  {
                  t.$elm.addClass('is-webgl');
                  distortionEffect.render();
              	}
                distortionEffect.change(current, newIndex);
            }
    ...
    masterPngDots.prototype = {
        ...
        build: function() {
            ...
            var onResize = function(){
                ...
                renderer.setSize(ww, wh);
                camera.aspect = ww / wh;
                camera.updatePortfolioionMatrix();
            };
...
(function($, window, document, undefined) {
    ...
    masterImageMorphing.prototype = {
        ...
        build: function() {
            ...
            if (target !== '') {
                var clip = t.$elm.find('svg clipPath').first();
                var viewbox = t.$elm.find('svg').attr('viewBox');
                var arr = viewbox.split(' ');
                var w = 1/ arr[2];
                var h = 1/ arr[3];
                clip.css('transform', 'scale(' + w + ',' + h + ')');
                $(target).css('clip-path', 'url("#' + clip.attr('id') + '")');
            }
            ...
            if (event == 'inView') {
                ...
                if (target == '') {
                    new IntersectionObserver(
                        function e(i, n) {
                            i.forEach(function (e) {
                                e.isIntersecting && (scroll(), n.unobserve(e.target));
                            });
                        }
                    ).observe(t.elm);
                } else {
...
(function($, window, document, undefined) {
    ...
    var masterImageTransition = function(elm, opts) {
        this.elm = elm;
        this.$elm = $(elm);
        this.opts = opts;
        this.config = this.$elm.data("config");
    ...
    masterImageTransition.prototype = {
        defaults: {
        },
        init: function() {
            this.args = $.extend({}, this.defaults, this.opts, this.config);
        ...
        build: function() {
            var t = this,
                images = t.$elm.find('.image'),
                current = 0,
                next = 1;
            setInterval(function() {
                t.change(current, next);
                current = next,
                (next == images.length - 1) ? next = 0 : next++
            }, 3000);   
        ...
        change: function(current, next) {
            ...
            gsap.set(images[next], {filter: 'none', scale: 1})
            gsap.to(images[current], 1, { opacity: 0, filter: 'blur(20px)' }),
            gsap.to(images[next], 1, { opacity: 1 })
        }
    };

    masterImageTransition.defaults = masterImageTransition.prototype.defaults;

    $.fn.masterImageTransition = function(opts) {
        return this.each(function() {
            new masterImageTransition(this, opts).init();
        });
    };
}(jQuery, window, document));
...
(function($, window, document, undefined) {
    ...
    var masterTextScroll = function(elm, opts) {
        this.elm = elm;
        this.$elm = $(elm);
        this.opts = opts;
        this.config = this.$elm.data("config");
    ...
    masterTextScroll.prototype = {
        ...
        build: function() {
            ...
            var t = this,
                title = t.elm,
                chars = t.$elm.find('.char').get(),
                words = t.$elm.find('.word').get(),
                target = chars,
                effect = t.args.effect,
                duration = t.args.speed,
                stagger = t.args.stagger,
                scrub = t.args.scrub,
                args = [];

            t.initSmoothScrolling();
            (t.args.split == 'word') && (target = words)
            gsap.set(target, {opacity: 0})
                    ...
                    for (const word of words) {
                        args = {
                            from: { 
                                'will-change': 'transform', 
                                scaleX: 0,
                                x: (_, elm) => window.innerWidth/2 - elm.offsetLeft - elm.offsetWidth/2
                            }, 
                            to: {
                                ease: 'power1.inOut',
                                scaleX: 1,
            ...
            (scrub !== 'true') && (args.to.scrollTrigger.scrub = false),
            ((parseInt(duration) !== 0) && !isNaN(parseInt(duration))) && (args.to.duration = duration),
            ((parseInt(stagger) !== 0) && !isNaN(parseInt(stagger))) && (args.to.stagger = stagger)
            switch(effect) {
                case 'effect8':
                    ...
                    chars.forEach((char, position) => {
                        ...
                        {
                            duration: ((parseInt(duration) !== 0) && !isNaN(parseInt(duration))) ? duration : 0.05,
                    ...
                    break;
                default:
                    gsap.fromTo(target, args.from, args.to);

            };
            
        },
        wrapElement: function(elems, wrapType, wrapClass) {
            elems.forEach(char => {
    ...
    masterTextScroll.defaults = masterTextScroll.prototype.defaults;

    $.fn.masterTextScroll = function(opts) {
        return this.each(function() {
            new masterTextScroll(this, opts).init();
        });
    };
}(jQuery, window, document));